        ...
        <Table.Tr key={element.name}>
          ...
          <Table.Td>
            <select
              ...
              onChange={(e) => toggle(e, element)}
              className={`${element.status ? "bg-green-1" : "bg-gray-0"} border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full max-w-[5.25rem] py-[ 0.0625rem] px-[0.25rem]`}
            >
          ...
          <Table.Td className="text-right ">
            <span className="text-black-1 text-bold">{element.id}</span>{" "}
          </Table.Td>
        </Table.Tr>
      ))
    ) : (
      <Table.Tr key={"333"}>لا يوجد</Table.Tr>
    );
